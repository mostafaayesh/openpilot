...
class CarController():
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd,
    ...
    lkas_active = enabled and not CS.steer_not_allowed and CS.lateralEnabled
    ...
    if (frame % 10) == 0:
      ...
      hud = HUDData(int(pcm_accel), (int(round(hud_v_cruise)) if hud_car != 0 else 255), hud_car,
                    hud_lanes, fcw_display, acc_alert, steer_required, CS.read_distance_lines, CS.lateralEnabled and not lkas_active)
